<template>
  <div class="container">
    <div class="house" ref="house">
      <div class="items" v-for="(item, index) in houses" :key="index" @click="showHouse(item)">
        <div class="desc">
        ...
        <div class="house-flag">
          <img :src="`${imageCDN}${item.cname}.png`" alt="">
        </div>
      </div>
    </div>
    <div class="character">
      ...
      <div class="section">
        <div class="items" v-for="(item, index) in characters" :key="index" @click="showCharacter(item)">
          <img :src="`${imageCDN}${item.profile}?imageView2/1/w/280/h/440/format/jpg/q/75`" alt="">
          <div class="desc">
            <div class="cname">{{ item.cname }}</div>
            <div class="name">{{ item.name }}</div>
    ...
    <div class="city">
      ...
      <div class="intro">坐落于已知世界的最西端，狭长的维斯特洛大陆由北部的极地冰盖起向南延绵约3,000英里。绝境长城是一座巍峨挺立的不可逾越之物，横跨 300 英里，将最北的塞外地区与七大王国相互分离。一个统一的政治实体领导着南方的广阔土地，并形成九块相互联系又相互割据的区域。</div>
